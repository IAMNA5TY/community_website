const PLAYER_PAGES = ["city", "profile", "overview", "only-pixels", "discord"];
const GUEST_PAGES = ["city", "profile", "only-pixels", "discord"];
...
/** Same community tabs as Kick. Owner tools stay ones that work without Kick OAuth. */
const TWITCH_PLAYER_PAGES = PLAYER_PAGES.slice();
...
function getOwnerTwitchUsernames() {
  const raw = String(process.env.OWNER_TWITCH_USERNAMES || "iamna5ty").trim();
  return new Set(
    raw
      .split(",")
      .map((name) => String(name || "").trim().replace(/^@/, "").toLowerCase())
...
function filterHostPages(pages, options = {}) {
  const host =
    options.host ||
    options.hostname ||
    (typeof options.req?.hostname === "string" ? options.req.hostname : "");
  if (!isLocalDashboardHost(host)) {
    return pages.filter((page) => page !== "lighting");
  }
  return pages;
}

function getPublicPages(user, options = {}) {
  const provider = String(user?.provider || "kick").toLowerCase();
  const pages =
    provider === "twitch" ? TWITCH_PLAYER_PAGES.slice() : PLAYER_PAGES.slice();
  return filterHostPages(pages, options);
}

function getGuestPages(options = {}) {
  return filterHostPages(GUEST_PAGES.slice(), options);
}
...
function isPlayerAllowedApiPath(apiPath) {
  ...
  if (path === "/twitch/live") return true;
  return false;
}

/** OBS / Streamlabs overlays poll these without an owner session. */
const PUBLIC_OVERLAY_API_PATHS = new Set([
  "/health",
  "/state",
  "/workout",
  "/workout/events",
...
function isPublicOverlayApiPath(apiPath) {
  const path = String(apiPath || "").replace(/\/+$/, "") || "/";
  return PUBLIC_OVERLAY_API_PATHS.has(path);
}
