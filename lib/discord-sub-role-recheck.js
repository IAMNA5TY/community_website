...
async function runRecheck(kickSubscriberStore, options = {}) {
  ...
  try {
    ...
    lastRun = {
      ...
      mode: "status-only",
      note:
        "Auto role removal is off. Kick chat badges still mark people eligible to claim; roles stay until removed by hand.",
...
function startRecheckLoop(kickSubscriberStore) {
  ...
  console.log(
    "[discord-recheck] auto role removal disabled — chat badges still used for claim eligibility"
  );
  lastRun = {
