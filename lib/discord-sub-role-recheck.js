...
// Require several no-badge sightings across a delay window before any revoke.
const noBadgeRevokePending = new Map(); // discordId -> { count, firstAt, lastAt, kickUsername, kickUserId }

function getChatRevokeConfirmCount() {
  return Math.max(
    3,
    Number(process.env.DISCORD_CHAT_REVOKE_CONFIRM_COUNT || 5) || 5
  );
}

function getChatRevokeConfirmMs() {
  return Math.max(
    10 * 60 * 1000,
    Number(process.env.DISCORD_CHAT_REVOKE_CONFIRM_MS || 45 * 60 * 1000) ||
      45 * 60 * 1000
  );
}
...
async function revokeFromChatEvidence(
  ...
  if (kickSubscriberStore.isChannelOwner(kickUserId, kickUsername)) {
    noBadgeRevokePending.delete(id);
  ...
  // Debounce: badge reads flap (no badge → has badge → no badge). Require many
  // consecutive no-badge hits across a long window before removing.
  const now = Date.now();
  const needCount = getChatRevokeConfirmCount();
  const needMs = getChatRevokeConfirmMs();
  const prev = noBadgeRevokePending.get(id) || {
    count: 0,
    firstAt: now,
    lastAt: 0,
    kickUsername,
  ...
  // Reset if the window expired without enough confirms.
  if (prev.lastAt && now - prev.firstAt > needMs * 2) {
    prev.count = 0;
    prev.firstAt = now;
  }
  prev.count += 1;
  prev.lastAt = now;
  prev.kickUsername = kickUsername || prev.kickUsername;
  prev.kickUserId = kickUserId || prev.kickUserId;
  noBadgeRevokePending.set(id, prev);

  const waitedMs = now - prev.firstAt;
  if (prev.count < needCount || waitedMs < needMs) {
    console.log(
      `[discord-recheck] no-badge pending for ${kickUsername || id}: ${prev.count}/${needCount} over ${Math.round(waitedMs / 1000)}s (need ${Math.round(needMs / 1000)}s)`
    );
    return {
      ok: true,
      skipped: true,
      reason: "awaiting-confirmations",
      pendingCount: prev.count,
      needCount,
      waitedMs,
      needMs,
    };
  ...
  try {
    ...
    console.log(
      `[discord-recheck] revoked sub role for ${kickUsername || id} after ${prev.count} no-badge confirms over ${Math.round(waitedMs / 1000)}s`
    );
...
/** Clear pending no-badge revoke when we see a subscriber badge again. */
function clearChatRevokePending(discordId) {
  const id = String(discordId || "").trim();
  if (id) noBadgeRevokePending.delete(id);
}
