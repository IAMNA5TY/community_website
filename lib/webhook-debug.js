...
function recordHit({ eventType, headers = {}, valid, reason, payload, channelId, stored }) {
  ...
  const fromKick = Boolean(headers["kick-event-message-id"]);

  if (fromKick) {
    stats.kickHits = (stats.kickHits || 0) + 1;
    stats.lastKickHitAt = stats.lastHitAt;
  } else {
    stats.otherHits = (stats.otherHits || 0) + 1;
  }
