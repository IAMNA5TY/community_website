...
async function handleSubGoalCommand(broadcasterUserId, content, payload, kickConfig) {
  ...
  if (parsed.action === "show") {
    const state = streamSubGoalState.load();
  ...
  if (!isModerator(payload, broadcasterUserId)) {
    await sendChatMessage(
      broadcasterUserId,
      "Mods can reset or adjust the stream sub goal.",
      kickConfig
    );
  ...
  if (parsed.action === "reset") {
    response = `Stream sub goal reset — ${result.state.count}/${result.state.goal} (${result.state.label}).`;
  } else if (parsed.action === "setGoal") {
    response = `Sub goal set to ${result.state.goal} — ${streamSubGoalState.formatMessage(result.state)}`;
  } else if (parsed.action === "setLabel") {
    response = `Sub goal label set — ${streamSubGoalState.formatMessage(result.state)}`;
  }
  await sendChatMessage(broadcasterUserId, response, kickConfig);
