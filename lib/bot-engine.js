...
const streamSubGoalState = require("./stream-sub-goal-state");
const streamSubGoalEvents = require("./stream-sub-goal-events");
const streamSubathonState = require("./stream-subathon-state");
const streamSubathonEvents = require("./stream-subathon-events");
...
async function handleWalkCommand(broadcasterUserId, content, payload, kickConfig) {
  ...
  try {
    ...
    console.warn(`[bot] !walk reply failed after bank update: ${error.message}`);
  }
...
async function handleSubGoalCommand(broadcasterUserId, content, payload, kickConfig) {
  const parsed = streamSubGoalState.parseCommand(content);
  if (!parsed) {
    return null;
  }

  if (parsed.action === "show") {
    const state = streamSubGoalState.loadForDisplay();
    const response = streamSubGoalState.formatMessage(state);
    await sendChatMessage(broadcasterUserId, response, kickConfig);
    return { trigger: "!subgoal", response, subGoal: state };
  }

  if (!isModerator(payload, broadcasterUserId)) {
    await sendChatMessage(
      broadcasterUserId,
      "Mods can reset or adjust the stream timer.",
      kickConfig
    );
  ...
  const result = streamSubGoalState.applyAction({
  ...
  if (result.error) {
    await sendChatMessage(broadcasterUserId, result.error, kickConfig);
    return { trigger: "!subgoal", error: result.error };
  }

  streamSubGoalEvents.broadcast(result.state);
  let response = streamSubGoalState.formatMessage(result.state);
  if (parsed.action === "reset") {
    response = `Stream timer reset — ${result.state.displayTime} / ${result.state.maxDisplayTime} (starts at 4h, +${result.state.minutesPerSub} min/sub, max 12h).`;
  } else if (parsed.action === "start") {
    response = `Stream timer started — ${result.state.displayTime} left.`;
  } else if (parsed.action === "stop") {
    response = `Stream timer paused — ${result.state.displayTime} banked.`;
  } else if (parsed.action === "setLabel") {
    response = `Timer label set — ${streamSubGoalState.formatMessage(result.state)}`;
  }
  await sendChatMessage(broadcasterUserId, response, kickConfig);
  return { trigger: "!subgoal", response, subGoal: result.state };
}

async function handleSubathonCommand(broadcasterUserId, content, payload, kickConfig) {
  const parsed = streamSubathonState.parseCommand(content);
  if (!parsed) {
    return null;
  }

  if (parsed.action === "show") {
    const state = streamSubathonState.loadForDisplay();
    const response = streamSubathonState.formatMessage(state);
    await sendChatMessage(broadcasterUserId, response, kickConfig);
    return { trigger: "!subathon", response, subathon: state };
  }

  if (!isModerator(payload, broadcasterUserId)) {
    await sendChatMessage(
      broadcasterUserId,
      "Mods can start, pause, or add time to the subathon.",
      kickConfig
    );
    return { trigger: "!subathon", denied: true };
  }

  const username = payload.sender?.username || payload.username || "mod";
  const result = streamSubathonState.applyAction({
    ...parsed,
    by: username,
  });
  if (result.error) {
    await sendChatMessage(broadcasterUserId, result.error, kickConfig);
    return { trigger: "!subathon", error: result.error };
  }

  streamSubathonEvents.broadcast(result.state);
  let response = streamSubathonState.formatMessage(result.state);
  if (parsed.action === "reset") {
    response = `Subathon reset — ${result.state.displayTime} on the clock, +${result.state.minutesPerSub} min/sub.`;
  } else if (parsed.action === "seed") {
    response = `Subathon opened on ${result.state.count} subs — ${result.state.displayTime} on the clock (+${result.state.minutesPerSub} min/sub).`;
  } else if (parsed.action === "start") {
    response = `Subathon started — ${result.state.displayTime} left.`;
  } else if (parsed.action === "stop") {
    response = `Subathon paused — ${result.state.displayTime} banked.`;
  } else if (parsed.action === "add" || parsed.action === "addMinutes") {
    response = `Subathon +${Math.round((result.state.lastAddedSeconds || 0) / 60)} min — ${result.state.displayTime} left.`;
  }
  await sendChatMessage(broadcasterUserId, response, kickConfig);
  return { trigger: "!subathon", response, subathon: result.state };
}
...
async function handleChatMessage(broadcasterUserId, payload, kickConfig) {
  ...
  // Apply treadmill commands before any other awaits so !walk can't lose a race.
  ...
  const subGoalResult = await handleSubGoalCommand(
    broadcasterUserId,
    content,
    payload,
    kickConfig
  ...
  const subathonResult = await handleSubathonCommand(
    broadcasterUserId,
    content,
    payload,
    kickConfig
...
function handleSubscriptionEvent(quantity = 1) {
  ...
  const subGoal = streamSubGoalState.save(
    streamSubGoalState.addSubs(quantity, "kick-webhook")
  );
  streamSubGoalEvents.broadcast(subGoal);
  const subathon = streamSubathonState.save(
    streamSubathonState.addSubs(quantity, "kick-sub")
  );
  streamSubathonEvents.broadcast(subathon);
}
