...
function extractChatPayload(messageData) {
  ...
  if (!content) {
    const emotes = messageData.emotes || messageData.emote || [];
    if (Array.isArray(emotes) && emotes.length) {
      content = emotes
        .map((emote) => {
          const id = emote?.id ?? emote?.emote_id ?? emote?.emoteId;
          const name = emote?.name ?? emote?.emote_name ?? emote?.emoteName;
  ...
  onMessage(raw) {
    ...
    if (recorded?.controlEvent?.action === "balance") {
      require("./kick-balance-reply")
        .maybeReplyBalance(this.slug, username, recorded.controlEvent)
        .catch((error) => {
          console.warn(`[pusher-monitor] balance reply failed: ${error.message}`);
        });
    }
    ...
    try {
      ...
      try {
        const rawBadges =
          ...
          messageData.sender?.identity?.badges ??
          messageData.sender?.badges;
        // undefined = Kick omitted identity (unknown). [] = confirmed no badges.
        const badges = rawBadges;
        const explicitSubscriber =
          payload.sender?.is_subscriber === true ||
          payload.sender?.isSubscriber === true
            ? true
            : payload.sender?.is_subscriber === false ||
                payload.sender?.isSubscriber === false
              ? false
              : null;
        const isSubscriber = Boolean(
          explicitSubscriber === true ||
        ...
        const kickSubscriberStore = require("./kick-subscriber-store");
        const observed = kickSubscriberStore.observeChatSubscriberBadge({
          username,
          userId: payload.sender?.user_id || null,
          isSubscriber,
          badges,
          explicitSubscriber,
        });
        if (observed?.action === "renewed" || observed?.action === "owner") {
          const link =
            kickSubscriberStore.getLinkForKickUser?.(
              observed.kickUserId || payload.sender?.user_id || null
            ) || null;
          if (link?.discordId) {
            require("./discord-sub-role-recheck").clearChatRevokePending?.(
              link.discordId
            );
          }
        }
        if (observed?.action === "should-revoke") {
          require("./discord-sub-role-recheck")
            .revokeFromChatEvidence(kickSubscriberStore, observed)
            .catch((error) => {
              console.warn(
                `[pusher-monitor] chat badge revoke failed: ${error.message}`
              );
            });
        }
      } catch {
  ...
  async connectSocket() {
    ...
    await new Promise((resolve, reject) => {
      ...
      ws.on("open", () => {
        this.connected = true;
        this.reconnectDelay = 5000;
        // Only the v2 chatroom channel — legacy + channel/livestream can echo the same event.
