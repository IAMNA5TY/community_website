...
class StreamMonitor {
  ...
  onMessage(raw) {
    ...
    // Owner-channel chat commands (!start, !walk, etc.) — no chat webhook needed.
    this.forwardOwnerCommands(messageData, username, content);
  }

  forwardOwnerCommands(messageData, username, content) {
    const ownerSlug = String(
      process.env.KICK_OWNER_SLUG ||
        process.env.DEFAULT_BROADCASTER_SLUG ||
        "na5ty"
    ).toLowerCase();
    if (this.slug !== ownerSlug) return;
...
class KickPusherMonitor {
  ...
  async drainStartQueue() {
    ...
    while (this.startQueue.length) {
      ...
      if (!monitor.chatroomId && Date.now() - failedAt < 20000) {
        continue;
      }

      try {
        // Hard cap so one hung Kick/Cloudflare lookup cannot stall every live partner.
        await Promise.race([
          monitor.start(),
          new Promise((_, reject) =>
            setTimeout(() => reject(new Error("monitor start timeout")), 45000)
          ),
        ]);
      ...
      // Slow stagger — Kick/Cloudflare blocks bursts (first few OK, rest "no chatroom id").
  ...
  queueSlug(slug, { front = false } = {}) {
    if (!slug) return;
    if (this.startQueue.includes(slug)) return;
