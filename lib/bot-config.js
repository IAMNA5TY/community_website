...
const CONFIG_PATH = path.join(getDataDir(), "bot-config.json");
const LEGACY_CONFIG_PATH = path.join(__dirname, "..", "data", "bot-config.json");
...
function migrateLegacyConfig() {
  ...
  try {
    ...
    console.log("[bot-config] migrated bot-config.json into DATA_DIR");
  } catch (error) {
    console.warn("[bot-config] legacy migrate failed:", error.message);
  }
}
