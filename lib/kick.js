...
async function getAllEventSubscriptions(accessToken) {
  const data = await kickFetchSafe(`/events/subscriptions`, accessToken);
  return data?.data || [];
}

/** Delete leftover chat.message.sent subs — chat is Pusher-only now. */
async function purgeChatMessageSubscriptions(accessToken, broadcasterUserIds = null) {
  let subs = [];
  if (Array.isArray(broadcasterUserIds) && broadcasterUserIds.length) {
    for (const id of broadcasterUserIds) {
      const rows = await getEventSubscriptions(accessToken, id);
      subs.push(...rows);
    }
  } else {
    subs = await getAllEventSubscriptions(accessToken);
  }

  const chatIds = [
    ...new Set(
      subs
        .filter((sub) => String(sub.event || sub.name || "") === "chat.message.sent")
        .map((sub) => sub.id)
        .filter(Boolean)
    ),
  ];
  ...
  // Kick delete endpoint accepts batches; chunk to be safe.
  for (let i = 0; i < chatIds.length; i += 25) {
    await deleteEventSubscriptions(accessToken, chatIds.slice(i, i + 25));
    await new Promise((resolve) => setTimeout(resolve, 500));
  }

  return { deleted: chatIds.length, ids: chatIds };
}
...
async function getDashboard(req, config, options = {}) {
  const accessToken = await ensureAccessToken(req, config);
  const user = req.session.user;
  const cacheKey = String(user.profile?.id || "");
  const maxAgeMs = Math.max(
    5_000,
    Number(options.maxAgeMs ?? process.env.KICK_DASHBOARD_CACHE_MS ?? 45_000) || 45_000
  );

  if (
    !options.force &&
    cacheKey &&
    dashboardCache.has(cacheKey) &&
    Date.now() - dashboardCache.get(cacheKey).at < maxAgeMs
  ) {
    return dashboardCache.get(cacheKey).value;
  }

  // Skip events/subscriptions on the hot dashboard path unless explicitly requested.
  // Subscription setup is handled separately (login / boot / reregister).
  ...
  const value = {
    ...
    apiSources: {
      ...
      redemptions: pendingGroups.length || acceptedGroups.length
        ...
        : null,
      eventSubscriptions: includeSubscriptions
