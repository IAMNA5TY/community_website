...
async function subscribeToChannelEvents(accessToken, broadcasterUserId) {
  const events = [
  ...
  const existing = broadcasterUserId
    ...
    : [];
  const existingEvents = new Set(existing.map((sub) => sub.event));
  const missing = events.filter((event) => !existingEvents.has(event.name));
