...
const DEFAULT_STATE = {
  ...
  restoredHalvedTime: false,
  undidHalvedRestore: false,
};
...
function undoRestoredHours(next) {
  const sub = OPENING_SUBS * DEFAULT_SECONDS_PER_SUB;
  if (next.isRunning && next.endsAt) {
    next.endsAt -= sub * 1000;
    next.pausedRemaining = clampRemaining((next.endsAt - Date.now()) / 1000, next.maxSeconds);
  } else {
    next.pausedRemaining = clampRemaining((Number(next.pausedRemaining) || 0) - sub, next.maxSeconds);
  }
  next.lastAddedSeconds = 0;
  if (next.lastSubBy === "restored leftover") {
    next.lastSubBy = null;
    next.lastSubAt = null;
  }
  next.undidHalvedRestore = true;
  next.restoredHalvedTime = true;
}
...
function migrate(raw) {
  ...
  // Take back the one-time +25h restore — leave the clock at 10:09:29.
  if (raw.restoredHalvedTime === true && !raw.undidHalvedRestore && raw.lastSubBy === "restored leftover") {
    undoRestoredHours(next);
  }
  ...
  next.restoredHalvedTime = Boolean(next.restoredHalvedTime);
  next.undidHalvedRestore = Boolean(next.undidHalvedRestore);
...
function load() {
  ...
  const halved =
    next.scaledToFiveMin && raw.scaledToFiveMin !== true && parseInt(raw.secondsPerSub, 10) === TEN_MIN_SECONDS;
  const undid = next.undidHalvedRestore && raw.undidHalvedRestore !== true && raw.lastSubBy === "restored leftover";
  if (halved || undid) writeState(next);
  return next;
}
