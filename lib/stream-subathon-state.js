...
function addRestoredHours(next) {
  const add = OPENING_SUBS * DEFAULT_SECONDS_PER_SUB;
  if (next.isRunning && next.endsAt) {
    next.endsAt += add * 1000;
    next.pausedRemaining = clampRemaining((next.endsAt - Date.now()) / 1000, next.maxSeconds);
  } else {
    next.pausedRemaining = clampRemaining((Number(next.pausedRemaining) || 0) + add, next.maxSeconds);
  }
  next.lastAddedSeconds = add;
  next.lastSubAt = new Date().toISOString();
  next.lastSubBy = "restored leftover";
...
function migrate(raw) {
  ...
  // Put back the 25h that cut took off already-converted clocks. Skip the same pass as the half.
  if (raw.scaledToFiveMin === true && !raw.restoredHalvedTime) {
    addRestoredHours(next);
  }
...
function load() {
  ...
  const restored = next.restoredHalvedTime && raw.restoredHalvedTime !== true && raw.scaledToFiveMin === true;
  if (halved || restored) writeState(next);
  return next;
}
