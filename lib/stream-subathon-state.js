...
const DEFAULT_SECONDS_PER_SUB = 10 * 60;
const LEGACY_SECONDS_PER_SUB = 5 * 60;
...
function migrate(raw) {
  ...
  // 5 min/sub was the first default — bump saved clocks to 10 so a 300-gift feels bigger.
  if (parseInt(next.secondsPerSub, 10) === LEGACY_SECONDS_PER_SUB) {
...
function shouldOpenOnGiftStack(state) {
  ...
  if (state.count === 0 && remaining <= DEFAULT_START_SECONDS + 30) {
    return true;
  }
  const leftoverFiveMinOpen = OPENING_SUBS * LEGACY_SECONDS_PER_SUB;
  if (
    state.count === OPENING_SUBS &&
    state.secondsPerSub === DEFAULT_SECONDS_PER_SUB &&
    Math.abs(remaining - leftoverFiveMinOpen) <= 15 * 60
  ) {
    return true;
  }
  return false;
...
/** Open the clock on a gifted stack (e.g. 300 subs × 10 min = 50:00:00). */
