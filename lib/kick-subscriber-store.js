...
/** Mark Kick roster entry as expired / not a sub (seen in chat without badge). */
 ...
 * - Without badge → only revoke Discord role if we previously saw them WITH a badge
 *   (avoids yanking manual grants / owner when Kick omits badge fields)
 */
...
function recordChatPresence({
  ...
  store.chatPresence[key] = {
    ...
    kickUserId: userId ? String(userId) : prev.kickUserId || null,
    lastSeenAt: now,
    lastHadSubscriberBadge: Boolean(isSubscriber),
    lastBadges: badgeTypes.length ? badgeTypes : prev.lastBadges || [],
    withBadgeAt: isSubscriber ? now : prev.withBadgeAt || null,
    withoutBadgeAt: isSubscriber ? prev.withoutBadgeAt || null : now,
...
function observeChatSubscriberBadge({
  ...
  // Don't revoke until we've positively seen a sub badge on this person at least once.
  // Stops false removes right after claim when Kick chat payloads omit badges.
