 ...
 * - Without badge → clear roster; if they hold Discord sub role, caller should revoke
 */
function observeChatSubscriberBadge({
  ...
  if (isChannelOwner(userId, username)) {
    return { action: "owner", kickUsername: name, kickUserId: userId };
  }

  if (isSubscriber) {
    markSubscriberFromChat({ username, userId });
    return { action: "renewed", kickUsername: name, kickUserId: userId };
  }

  clearSubscriberFromChat(username, userId);
  const hit = findActiveGrantForKick(userId, username);
  if (!hit?.grant?.discordId) {
    return { action: "not-sub", kickUsername: name, kickUserId: userId };
  }
