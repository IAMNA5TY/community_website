...
function getPublicStatusForKickUser(kickUserId, kickUsername) {
  ...
  const active = isActiveSubscriber(kickUsername) || isActiveSubscriber(kickUserId);
  ...
  return {
    ...
    note: active
      ? "Kick shows you as an active subscriber in our webhook/chat roster."
      : "No active paid/gifted sub found yet. Sub or chat while subbed so Kick can register you.",
  };
}
