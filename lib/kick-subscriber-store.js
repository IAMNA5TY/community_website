...
const MAX_CHAT_PRESENCE = Math.max(
  200,
  Number(process.env.DISCORD_CHAT_PRESENCE_MAX || 3000) || 3000
);
const CHAT_PRESENCE_TTL_MS = Math.max(
  24 * 60 * 60 * 1000,
  Number(process.env.DISCORD_CHAT_PRESENCE_TTL_MS || 14 * 24 * 60 * 60 * 1000) ||
    14 * 24 * 60 * 60 * 1000
);
    ...
    return {
      ...
      chatPresence:
        parsed.chatPresence && typeof parsed.chatPresence === "object"
          ...
          : {},
      claimAttempts:
        parsed.claimAttempts && typeof parsed.claimAttempts === "object"
          ? parsed.claimAttempts
          : {},
...
function pruneChatPresence(store) {
  const presence = store.chatPresence || {};
  const keys = Object.keys(presence);
  if (keys.length < MAX_CHAT_PRESENCE && keys.length < 50) return;

  const cutoff = Date.now() - CHAT_PRESENCE_TTL_MS;
  const entries = Object.entries(presence).filter(([, row]) => {
    const seen = new Date(row?.lastSeenAt || 0).getTime();
    return Number.isFinite(seen) && seen >= cutoff;
  });
  if (entries.length > MAX_CHAT_PRESENCE) {
    entries.sort(
      (a, b) =>
        new Date(b[1]?.lastSeenAt || 0).getTime() -
        new Date(a[1]?.lastSeenAt || 0).getTime()
    );
    entries.length = MAX_CHAT_PRESENCE;
  }
  if (entries.length !== keys.length) {
    store.chatPresence = Object.fromEntries(entries);
  }
}
...
function mutateStore(mutator) {
  ...
  const presenceCount = Object.keys(store.chatPresence || {}).length;
  if (presenceCount >= MAX_CHAT_PRESENCE || presenceCount % 50 === 0) {
    pruneChatPresence(store);
  }
  writeStore(store);
  return result;
}
...
function observeChatSubscriberBadge({
  ...
  // Auto-revoke is off unless DISCORD_CHAT_ROLE_REVOKE=1 (handled in revokeFromChatEvidence).
  return {
...
function listChatBadgeRoster(limit = 200) {
  ...
  const upsertRow = (key, patch) => {
    ...
    byName.set(key, { ...prev, ...patch, username: patch.username || prev.username || key });
  };

  for (const row of Object.values(store.chatPresence || {})) {
    ...
    if (prev) {
      if (active) {
        upsertRow(key, {
          ...
          lastBadgeStatus: prev.lastBadgeStatus || "subscriber",
          lastBadges:
            prev.lastBadges?.length ? prev.lastBadges : ["subscriber (roster)"],
          kickUserId: prev.kickUserId || row.kickUserId || null,
        });
      }
      continue;
    }
  ...
  // Discord-linked Kick users who never showed in chat yet
  for (const link of Object.values(store.links || {})) {
    const key = normalizeUsername(link.kickUsername);
    if (!key) continue;
    if (byName.has(key)) {
      const prev = byName.get(key);
      upsertRow(key, {
        kickUserId: prev.kickUserId || link.kickUserId || null,
        lastSeenAt: prev.lastSeenAt || link.linkedAt || null,
        source: prev.source || "linked",
      });
      continue;
    }
    upsertRow(key, {
      username: link.kickUsername || key,
      kickUserId: link.kickUserId || null,
      lastSeenAt: link.linkedAt || null,
      hasSubscriberBadge: false,
      lastBadgeStatus: "linked-only",
      lastBadges: [],
      source: "linked",
    });
  }

  // People who clicked Get Subscriber Role / Claim (success or fail)
  for (const attempt of Object.values(store.claimAttempts || {})) {
    const key = normalizeUsername(attempt.kickUsername);
    if (!key) continue;
    const prev = byName.get(key);
    upsertRow(key, {
      username: attempt.kickUsername || key,
      kickUserId: attempt.kickUserId || prev?.kickUserId || null,
      lastSeenAt:
        attempt.at && (!prev?.lastSeenAt || attempt.at > prev.lastSeenAt)
          ? attempt.at
          : prev?.lastSeenAt || attempt.at || null,
      source: prev?.source || "claim-attempt",
      lastClaimAt: attempt.at || null,
      lastClaimResult: attempt.result || null,
      lastClaimReason: attempt.reason || null,
      hasSubscriberBadge:
        prev?.hasSubscriberBadge ||
        attempt.result === "granted" ||
        attempt.result === "already-eligible",
    });
  }

  // Active/inactive grants by Kick username
  for (const grant of Object.values(store.grants || {})) {
    const key = normalizeUsername(grant.kickUsername);
    if (!key) continue;
    const prev = byName.get(key);
    if (prev) continue;
    upsertRow(key, {
      username: grant.kickUsername || key,
      kickUserId: grant.kickUserId || null,
      lastSeenAt: grant.lastCheckedAt || grant.grantedAt || null,
      hasSubscriberBadge: Boolean(grant.active),
      lastBadgeStatus: grant.active ? "subscriber" : null,
      lastBadges: grant.active ? ["subscriber (grant)"] : [],
      source: "grant",
    });
  }
  ...
  const rows = [...byName.values()].map((row) => {
    ...
    const grantByName =
      ...
      null;
    const attempt =
      store.claimAttempts[normalizeUsername(row.username)] ||
      (link?.discordId ? store.claimAttempts[`discord:${link.discordId}`] : null) ||
      null;
    return {
      ...row,
      isOwner: isChannelOwner(row.kickUserId, row.username),
      discordLinked: Boolean(link?.discordId || grantByName?.discordId),
      discordId: String(link?.discordId || grantByName?.discordId || "") || null,
      discordUsername: link?.discordUsername || null,
      roleGranted: Boolean(grantByName?.active),
      lastClaimAt: row.lastClaimAt || attempt?.at || null,
      lastClaimResult: row.lastClaimResult || attempt?.result || null,
      lastClaimReason: row.lastClaimReason || attempt?.reason || null,
    };
  });

  rows.sort((a, b) => {
    const aTry = a.lastClaimAt || a.lastSeenAt || "";
    const bTry = b.lastClaimAt || b.lastSeenAt || "";
    return String(bTry).localeCompare(String(aTry));
  });

  return rows.slice(0, Math.max(1, Math.min(500, limit)));
}

function recordClaimAttempt({
  discordId = null,
  kickUsername = null,
  kickUserId = null,
  result = "unknown",
  ...
  const key = name || (dId ? `discord:${dId}` : "");
  if (!key) return null;

  return mutateStore((store) => {
    store.claimAttempts = store.claimAttempts || {};
    const prev = store.claimAttempts[key] || {};
    store.claimAttempts[key] = {
      key,
      kickUsername: name || prev.kickUsername || null,
      kickUserId: kickUserId ? String(kickUserId) : prev.kickUserId || null,
      discordId: dId || prev.discordId || null,
      result: String(result || "unknown"),
      reason: reason ? String(reason).slice(0, 240) : null,
      at: new Date().toISOString(),
      count: Number(prev.count || 0) + 1,
    };
    return store.claimAttempts[key];
  });
}
...
function getChatPresence(username) {
  const key = normalizeUsername(username);
  if (!key) return null;
  const store = readStore();
  return store.chatPresence?.[key] || null;
}

/**
 * Decide whether Kick Supporter should stay on a Discord user.
 * Used when RoleLogic / mods / other bots strip the role incorrectly.
 */
function resolveSubRoleKeepReason(discordUserId) {
  const id = String(discordUserId || "").trim();
  if (!id) return { keep: false, reason: "no-discord-id" };

  const grant = getGrant(id);
  const link = getLinkForDiscordId(id);
  ...
  if (grant?.active) {
    return {
      ...
      grant,
      link,
      kickUsername: grant.kickUsername || link?.kickUsername || null,
      kickUserId: grant.kickUserId || link?.kickUserId || null,
    };
  }

  if (link) {
    const elig = isEligibleForSubRole(link.kickUserId, link.kickUsername);
    if (elig.eligible) {
      return {
        keep: true,
        reason: elig.reason === "owner" ? "owner" : "eligible-subscriber",
        grant,
        link,
    ...
    const presence = getChatPresence(link.kickUsername);
    if (presence?.lastHadSubscriberBadge) {
      return {
        keep: true,
        reason: "chat-subscriber-badge",
        grant,
        link,
        kickUsername: link.kickUsername,
        kickUserId: link.kickUserId || presence.kickUserId || null,
      };
    }

    // Grant was cleared by a false external remove earlier — revive if still linked
    // and we have any positive sub evidence on the Kick side.
    if (
      grant &&
      grant.active === false &&
      (grant.revokeReason === "manual-discord-remove" ||
        String(grant.revokeReason || "").includes("rolelogic") ||
        String(grant.revokeReason || "").includes("external"))
    ) {
      if (presence?.lastBadgeStatus === "subscriber" || presence?.withBadgeAt) {
        return {
          keep: true,
          reason: "revive-after-external-remove",
          grant,
          link,
          kickUsername: link.kickUsername,
          kickUserId: link.kickUserId,
