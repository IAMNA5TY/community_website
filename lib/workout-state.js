...
function writeState(state) {
  ...
  // Atomic replace — avoids truncated JSON if the process dies mid-write.
  ...
  if (bank > 0) {
    try {
      const lastGoodTmp = `${LAST_GOOD_PATH}.tmp`;
      fs.writeFileSync(lastGoodTmp, payload);
      fs.renameSync(lastGoodTmp, LAST_GOOD_PATH);
    } catch {
      /* ignore */
    }
  }
...
function protectBank(prev, merged, incoming, options = {}) {
  ...
  // Stale browser-source POSTs must not rewind !walk / sub bank increases.
  if (
    options.clientSync &&
    hasIncomingBank &&
    mergedBank < prevBank &&
    !incoming.intentionalBankClear
  ) {
    const incomingNonce = Number(incoming.stateNonce) || 0;
    const prevNonce = Number(prev.stateNonce) || 0;
    if (!incomingNonce || incomingNonce <= prevNonce) {
      console.warn(
        `[workout] blocked stale bank decrease ${mergedBank} → kept ${prevBank} (nonce ${incomingNonce}/${prevNonce})`
      );
      merged.minutesBank = prevBank;
      merged.minutesRemaining = Math.max(
        Number(merged.minutesRemaining) || 0,
        Number(prev.minutesRemaining) || prevBank
      );
      merged._secondsLeft = Math.max(
        Number(merged._secondsLeft) || 0,
        Number(prev._secondsLeft) || prevBank * 60
      );
      if (prev.isRunning && prev.treadmillEndAt) {
        merged.isRunning = true;
        merged.treadmillEndAt = prev.treadmillEndAt;
      }
    }
  }
...
function recoverAfterRestart() {
  ...
  // Repair absurd banks from the old remainingMs/60 bug (ms treated as near-seconds).
  const sec = Number(state._secondsLeft) || 0;
  const bank = Number(state.minutesBank) || 0;
  if (sec > 0 && bank > Math.ceil(sec / 60) + 1) {
    const fixedMin = Math.max(1, Math.ceil(sec / 60));
    const fixed = pauseKeepBank({
      ...state,
      _secondsLeft: sec,
      minutesBank: fixedMin,
      minutesRemaining: fixedMin,
    });
    writeState({ ...fixed, stateNonce: (state.stateNonce || 0) + 1 });
    console.warn(
      `[workout] repaired inflated bank ${bank} → ${fixedMin} min (matched ${_secondsLeftLabel(sec)})`
    );
    state = fixed;
  }
  ...
  if (remainingMs > 0) {
    // Still time left — pause so a deploy/crash can't race the clock to zero.
    // IMPORTANT: remainingMs is milliseconds → convert via seconds, not /60.
    const remainingSec = Math.max(1, Math.ceil(remainingMs / 1000));
    const remainingMin = Math.max(1, Math.ceil(remainingSec / 60));
    const paused = pauseKeepBank({
      ...state,
      _secondsLeft: remainingSec,
      minutesBank: remainingMin,
      minutesRemaining: remainingMin,
    });
    writeState({ ...paused, stateNonce: (state.stateNonce || 0) + 1 });
    console.log(
      `[workout] paused treadmill after restart — kept ${paused.minutesBank} min banked (${remainingSec}s left)`
    );
    return;
  }

  // Already past endAt after restart — never treat as a live finish (that wipes the bank).
  const paused = pauseKeepBank(state);
  writeState({ ...paused, stateNonce: (state.stateNonce || 0) + 1 });
  console.log(
    `[workout] recovered overdue run after restart — kept ${paused.minutesBank} min banked`
  );
}

function _secondsLeftLabel(sec) {
...
function adjustMinutes(delta, state = load()) {
  ...
  if (amount > 0) return addMinutes(amount, state);

  const next = { ...state };
  const removeSeconds = Math.abs(amount) * 60;

  if (next.isRunning && next.treadmillEndAt) {
    next.treadmillEndAt = Math.max(Date.now(), next.treadmillEndAt - removeSeconds * 1000);
    next._secondsLeft = getSeconds(next);
    next.minutesBank = Math.ceil((next._secondsLeft || 0) / 60);
  } else {
    const currentSeconds =
      next._secondsLeft > 0 ? next._secondsLeft : (next.minutesBank || 0) * 60;
    next._secondsLeft = Math.max(0, currentSeconds - removeSeconds);
    next.minutesBank = Math.ceil((next._secondsLeft || 0) / 60);
  }

  next.minutesRemaining = next.minutesBank;
  next.intentionalBankClear = next.minutesBank === 0;
  if (next.isRunning && (next._secondsLeft || 0) <= 0) {
    next.isRunning = false;
    next.treadmillEndAt = null;
  }
  return save(next, { allowZeroBank: next.minutesBank === 0 });
}
...
function parseWalkCommand(content) {
  ...
  if (/^!walk\s*$/i.test(trimmed)) return { action: "show" };

  // !walk +2, !walk+2, !walk - 3, !walk+
  const adjust = trimmed.match(/^!walk\s*([+-])\s*(\d+)?\s*$/i);
  if (!adjust) return null;

  const sign = adjust[1] === "+" ? 1 : -1;
  const amount = adjust[2] ? Number(adjust[2]) : 1;
