...
const STATE_PATH = path.join(__dirname, "..", "data", "workout-state.json");
...
function tick(state) {
  ...
  if (next.isRunning && next.treadmillEndAt) {
    const remaining = Math.max(0, Math.ceil((next.treadmillEndAt - Date.now()) / 1000));
...
function save(incoming, options = {}) {
  ...
  const state = tick({ ...prev, ...incoming });
