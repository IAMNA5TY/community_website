...
function send(op, d) {
  ...
  try {
    ...
    console.warn("[discord-gateway] send failed:", error.message);
  }
}
...
function trimMemberCache() {
  if (memberRoleCache.size <= MAX_MEMBER_CACHE) return;
  const excess = memberRoleCache.size - MAX_MEMBER_CACHE;
  let removed = 0;
  for (const key of memberRoleCache.keys()) {
    memberRoleCache.delete(key);
    seenMembers.delete(key);
    removed += 1;
...
function gatewayEnabled() {
  // Off by default — audit-log role watch covers manual add/remove without
  // privileged Server Members Intent. Set DISCORD_GATEWAY_ROLE_WATCH=1 to enable.
  return String(process.env.DISCORD_GATEWAY_ROLE_WATCH || "0").trim() === "1";
}
...
function start(store) {
  ...
  if (!gatewayEnabled()) {
    console.log(
      "[discord-gateway] skipped — use audit-log watch (set DISCORD_GATEWAY_ROLE_WATCH=1 to enable gateway)"
    );
    return;
  }
