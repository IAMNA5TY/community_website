...
function publicApplication(row) {
  ...
  return {
    ...
    twitchUsername: row.twitchUsername || null,
  };
}
...
function setTwitchUsername(kickUsername, twitchUsername) {
  const username = normalizeUsername(kickUsername);
  const twitch = normalizeUsername(twitchUsername);
  if (!username) return null;
  const store = readStore();
  const row = store.applications.find((item) => item.kickUsername === username);
  if (!row) return getApplication(username);
  row.twitchUsername = twitch || null;
  row.updatedAt = new Date().toISOString();
  writeStore(store);
  return publicApplication(row);
}
