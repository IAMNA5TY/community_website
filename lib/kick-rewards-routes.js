...
function createKickRewardsRouter(config) {
  ...
  function optionalApiKey(req, res, next) {
    ...
    if (req.session?.user && dashboardAccess.isDashboardOwner(req.session.user)) {
      return next();
    }
  ...
  function requireOwnerOrApiKey(req, res, next) {
    ...
    return res.status(403).json({ success: false, error: "Owner sign-in or API key required" });
  }
  ...
  router.get("/rewards/partners", requireOwnerOrApiKey, async (_req, res) => {
    try {
  ...
  router.post("/rewards/partners", requireOwnerOrApiKey, async (req, res) => {
    try {
  ...
  router.post("/rewards/partners/:slug/reconnect-chat", requireOwnerOrApiKey, async (req, res) => {
    try {
  ...
  router.delete("/monitored-streamers/:slug", requireOwnerOrApiKey, (req, res) => {
    try {
  ...
  router.delete("/rewards/partners/:slug", requireOwnerOrApiKey, (req, res) => {
    try {
      const result = kickRewardsStore.removeMonitoredStreamer(req.params.slug);
      kickPusherMonitor.refreshTargets();
      res.json({
