...
function createKickRewardsRouter(config) {
  ...
  // Alias for Only Pixels UI
  router.get("/rewards/partners", requireOwnerOrApiKey, (_req, res) => {
    res.json({
      success: true,
      streamers: kickRewardsStore.listMonitoredChannels(),
    });
  ...
  router.post("/rewards/partners", requireOwnerOrApiKey, async (req, res) => {
    try {
      ...
      kickPusherMonitor.refreshTargets();
      res.json({ success: true, streamer: entry, streamers: kickRewardsStore.listMonitoredChannels() });
    } catch (error) {
      res.status(400).json({ success: false, error: error.message });
    }
  });
