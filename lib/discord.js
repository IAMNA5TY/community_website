...
/**
 * Always post role grant/revoke announcements in the configured Discord panel channel.
 * Also tries the interaction channel / public follow-up when available.
 */
async function announceSubRoleChange({
  ...
  if (panelId) targets.add(panelId);
  const interactionChannel = String(
    channelId || interaction?.channel_id || ""
  ).trim();
  if (interactionChannel) targets.add(interactionChannel);

  const errors = [];

  // Public interaction follow-up (visible in the same channel as the button).
  if (interaction?.token && action === "granted") {
    try {
      await createInteractionFollowup(interaction, { content: text });
    } catch (error) {
      errors.push(`followup:${error.message}`);
    }
  }
  ...
  if (errors.length) {
    console.warn("[discord] role announcement issues:", errors.join(" | "));
  }
  return { ok: errors.length < targets.size + (interaction?.token ? 1 : 0), errors, text };
}
...
async function handleSubRoleButton(interaction, kickSubscriberStore) {
  ...
  if (!eligibility.eligible) {
    return {
      content:
        ...
        `Kick does not give us a live sub list. Type once in **na5ty** Kick chat while subbed (so we see your sub badge), ` +
        `or go to ${subscribeUrl} / wait for a gifted sub — then tap **Get Subscriber Role** again.`,
      components: [
        {
          type: 1,
          components: [
  ...
  try {
    ...
    // Public channel announcement for everyone (including owner, so you can test).
    await announceSubRoleChange({
      kickName,
      action: "granted",
    ...
    if (eligibility.reason === "owner") {
      return {
        content:
          "**You're signed in as the na5ty channel owner** — Discord linked, subscriber role granted, and a public thank-you was posted in the channel.",
        components: null,
      };
    }
