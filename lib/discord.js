...
/**
 * Move RoleLogic's bot role BELOW Kick Supporter so it can no longer strip it.
 * Our bot must sit above RoleLogic in the role list for this to work.
 */
async function blockRoleLogicFromManagingSubRole() {
  ...
  const candidateRoles = memberRoleIds
    .map((id) => byId.get(id))
    .filter(Boolean)
    // Ignore @everyone and the Kick Supporter role itself
    .filter((role) => String(role.id) !== String(cfg.guildId))
    .filter((role) => String(role.id) !== String(cfg.subRoleId));

  // Prefer a role named like RoleLogic; else highest-position role RoleLogic holds
  let roleLogicRole =
    candidateRoles.find((role) =>
      /role\s*logic/i.test(String(role.name || ""))
    ) || null;
  if (!roleLogicRole && candidateRoles.length) {
    roleLogicRole = candidateRoles.reduce((best, role) =>
      Number(role.position) > Number(best.position) ? role : best
    );
  }

  // Fallback: any guild role named RoleLogic (even if member payload is weird)
  if (!roleLogicRole) {
    roleLogicRole =
      roles.find(
        (role) =>
  ...
  if (!roleLogicRole) {
    return {
      ok: false,
      error:
        "Found RoleLogic in the server but could not identify its bot role. Drag RoleLogic below Kick Supporter manually in Server Settings → Roles.",
      roleLogicUserId: ROLELOGIC_BOT_USER_ID,
      memberRoleIds,
    };
  }

  const subPos = Number(subRole.position);
  const rlPos = Number(roleLogicRole.position);
  if (rlPos < subPos) {
    return {
      ok: true,
      alreadySafe: true,
      message: `RoleLogic role “${roleLogicRole.name}” is already below Kick Supporter — it should not be able to remove that role.`,
      roleLogicRole: {
        id: roleLogicRole.id,
  ...
  // Target: one slot below Kick Supporter
  ...
  try {
    ...
    return {
      ...
      error:
        error.message ||
        "Discord refused to move RoleLogic. Put the na5ty bot role ABOVE RoleLogic, then try again — or drag RoleLogic below Kick Supporter manually.",
      roleLogicRole: {
        id: roleLogicRole.id,
        name: roleLogicRole.name,
        position: rlPos,
  ...
  const updated =
    (Array.isArray(rolesAfter) ? rolesAfter : []).find(
      (role) => String(role.id) === String(roleLogicRole.id)
    ) || null;
  const newPos = Number(updated?.position ?? targetPosition);
  const safe = newPos < subPos;

  return {
    ok: safe,
    moved: true,
    message: safe
      ? `Moved RoleLogic role “${roleLogicRole.name}” below Kick Supporter. RoleLogic can no longer strip that role.`
      : `Tried to move RoleLogic but it is still at/above Kick Supporter (pos ${newPos}). Drag it below manually, or raise the na5ty bot above RoleLogic first.`,
    roleLogicRole: {
      id: roleLogicRole.id,
      name: roleLogicRole.name,
