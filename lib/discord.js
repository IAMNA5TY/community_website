...
const SUB_ROLE_OVERRIDE_PATH = () =>
  path.join(getDataDir(), "discord-sub-role.json");
...
let subRoleOverrideCache = null;
let subRoleOverrideLoaded = false;

function loadSubRoleOverride() {
  if (subRoleOverrideLoaded) return subRoleOverrideCache;
  subRoleOverrideLoaded = true;
  try {
    ensureDataDir();
    if (!fs.existsSync(SUB_ROLE_OVERRIDE_PATH())) {
      subRoleOverrideCache = null;
      return null;
    }
    const parsed = JSON.parse(fs.readFileSync(SUB_ROLE_OVERRIDE_PATH(), "utf8"));
    if (parsed?.roleId) {
      subRoleOverrideCache = {
        roleId: String(parsed.roleId),
        roleName: String(parsed.roleName || "na5ty Supporter"),
        createdAt: parsed.createdAt || null,
        replacedRoleId: parsed.replacedRoleId
          ? String(parsed.replacedRoleId)
          : null,
        reason: parsed.reason || null,
      };
      return subRoleOverrideCache;
    }
  } catch {
    subRoleOverrideCache = null;
  }
...
function saveSubRoleOverride(payload) {
  ensureDataDir();
  const next = {
    roleId: String(payload.roleId),
    roleName: String(payload.roleName || "na5ty Supporter"),
    createdAt: payload.createdAt || new Date().toISOString(),
    replacedRoleId: payload.replacedRoleId
      ? String(payload.replacedRoleId)
      : null,
    reason: payload.reason || "bypass-rolelogic",
    updatedAt: new Date().toISOString(),
  };
  fs.writeFileSync(SUB_ROLE_OVERRIDE_PATH(), JSON.stringify(next, null, 2));
  subRoleOverrideCache = next;
  subRoleOverrideLoaded = true;
  return next;
}

function configured() {
  const override = loadSubRoleOverride();
  return Boolean(
    process.env.DISCORD_CLIENT_ID &&
      process.env.DISCORD_CLIENT_SECRET &&
      process.env.DISCORD_BOT_TOKEN &&
      process.env.DISCORD_GUILD_ID &&
      (override?.roleId || process.env.DISCORD_SUB_ROLE_ID)
  );
}
...
function getConfig() {
  const override = loadSubRoleOverride();
  const envRoleId = String(process.env.DISCORD_SUB_ROLE_ID || "").trim();
  return {
    ...
    // Prefer na5ty-managed private role so RoleLogic can't strip it.
    subRoleId: String(override?.roleId || envRoleId || "").trim(),
    envSubRoleId: envRoleId,
    privateSubRole: override
      ? {
          roleId: override.roleId,
          roleName: override.roleName,
          createdAt: override.createdAt,
          replacedRoleId: override.replacedRoleId,
        }
      : null,
...
/**
 * Create/use a na5ty-owned Discord role that RoleLogic does not manage.
 * RoleLogic keeps fighting "Kick Supporter"; kick.bot-style private role fixes that.
 */
async function ensureNa5tyManagedSubRole({
  forceCreate = false,
  migrateGrants = true,
  kickSubscriberStore = null,
  roleName = null,
} = {}) {
  const cfg = getConfig();
  ...
  const existing = loadSubRoleOverride();
  if (existing?.roleId && !forceCreate) {
    // Verify role still exists
    try {
      const roles = await botFetch(`/guilds/${cfg.guildId}/roles`);
      const found = (Array.isArray(roles) ? roles : []).find(
        (role) => String(role.id) === String(existing.roleId)
      );
      if (found) {
        return {
          ok: true,
          ...
          roleName: found.name || existing.roleName,
          message: `Using na5ty-managed role “${found.name || existing.roleName}” (RoleLogic cannot strip this).`,
          privateSubRole: existing,
        };
      }
    } catch {
      /* recreate below */
  ...
  const name =
    String(roleName || process.env.DISCORD_PRIVATE_SUB_ROLE_NAME || "na5ty Supporter").trim() ||
    "na5ty Supporter";
  const replacedRoleId = cfg.envSubRoleId || cfg.subRoleId || null;

  let created;
  try {
    // Reuse existing guild role with same name if present.
    const roles = await botFetch(`/guilds/${cfg.guildId}/roles`);
    const reuse = (Array.isArray(roles) ? roles : []).find(
      (role) =>
        String(role.name || "").toLowerCase() === name.toLowerCase() &&
        String(role.id) !== String(cfg.guildId)
    );
    if (reuse) {
      created = reuse;
    } else {
      created = await botFetch(`/guilds/${cfg.guildId}/roles`, {
        method: "POST",
        body: JSON.stringify({
          name,
          color: 0x53fc18,
    ...
    return {
      ...
      error:
        error.message ||
        "Could not create na5ty Supporter role. Give the bot Manage Roles.",
      status: error.status,
    };
  }
  ...
  if (!roleId) {
    return { ok: false, error: "Discord returned no role id" };
  }

  const saved = saveSubRoleOverride({
    roleId,
    roleName: created.name || name,
    replacedRoleId,
    reason: "bypass-rolelogic-like-kickbot",
  });

  let migrated = 0;
  let migrateErrors = 0;
  if (migrateGrants && kickSubscriberStore?.listActiveGrants) {
    const grants = kickSubscriberStore.listActiveGrants();
    for (const grant of grants) {
      const discordId = String(grant.discordId || "").trim();
      if (!discordId) continue;
      try {
        // eslint-disable-next-line no-await-in-loop
        await ensureSubRole(discordId);
        migrated += 1;
        // eslint-disable-next-line no-await-in-loop
  ...
  try {
    await postChannelMessage(
      cfg.panelChannelId || DEFAULT_PANEL_CHANNEL_ID,
      `**na5ty sub role switched (kick.bot style)**\n` +
        `Supporters now get **${saved.roleName}** from na5ty.com — RoleLogic’s “Kick Supporter” strip loop can no longer take this away.\n` +
        `If sub channels used Kick Supporter, add **${saved.roleName}** to those channel permissions once.\n` +
        `Migrated ${migrated} grant(s)` +
        (migrateErrors ? ` (${migrateErrors} failed)` : "") +
        `.`
    );
  } catch {
  ...
  return {
    ...
    migrateErrors,
    message:
      `Created/using “${saved.roleName}” for na5ty claims. RoleLogic can keep fighting Kick Supporter — it won’t touch this role. ` +
      `Add “${saved.roleName}” to any sub-only channels. Migrated ${migrated} people` +
      (migrateErrors ? ` (${migrateErrors} failed)` : "") +
      `.`,
    privateSubRole: saved,
  };
}
    ...
    embeds: [
      {
        ...
        description:
          "To get the **na5ty Supporter** role, click **Get Subscriber Role** below.\n\n" +
          ...
          "This role is managed by na5ty.com (like the old Kick bot) — not RoleLogic.\n\n" +
