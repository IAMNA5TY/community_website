...
async function kickRoleLogicFromGuild(reason = "Fighting Kick Supporter role sync") {
  const cfg = getConfig();
  if (!cfg.guildId || !cfg.botToken) {
    return { ok: false, error: "Discord guild / bot token not configured" };
  }
  try {
    await botFetch(`/guilds/${cfg.guildId}/members/${ROLELOGIC_BOT_USER_ID}`, {
      method: "DELETE",
      headers: {
    ...
    return {
      ok: true,
      kicked: true,
      message: "Kicked RoleLogic from the Discord server.",
    };
  } catch (error) {
    if (error.status === 404) {
      return {
