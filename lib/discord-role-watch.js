...
function queueRoleLogicHardBlock(reason = 'rolelogic-remove') {
  if (String(process.env.DISCORD_BLOCK_ROLELOGIC || '1').trim() === '0') return;
  if (roleLogicBlockQueued) return;
  if (Date.now() - lastRoleLogicBlockAt < 60_000) return;
  ...
  setTimeout(() => {
    ...
    discord
      ...
      .then((result) => {
        console.log(
          `[discord-role-watch] RoleLogic hard-block after ${reason}: ${result.message || result.error || result.reason}`
        );
      })
      .catch((error) => {
        console.warn(
          `[discord-role-watch] RoleLogic hard-block failed:`,
          error.message
        );
      });
  }, 800);
...
async function handleEntry(entry, roleId, botUserId) {
  ...
  if (shouldKeep) {
    if (actor === 'RoleLogic') {
      queueRoleLogicHardBlock('rolelogic-strip-eligible');
    }
    try {
      // Small delay so RoleLogic's batch finishes before we put roles back,
      // then hard-block RoleLogic so the next ~11m cycle can't strip again.
      if (actor === 'RoleLogic') {
        await sleep(1500);
      }
  ...
  if (actor === 'RoleLogic') {
    queueRoleLogicHardBlock('rolelogic-strip-no-grant');
    console.log(
      `[discord-role-watch] RoleLogic remove for ${kickName} (${targetId}) — no keep evidence; hard-blocking RoleLogic`
    );
    return { handled: false, restored: false };
  }
