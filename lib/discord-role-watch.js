...
async function handleEntry(entry, roleId, botUserId) {
  ...
  // REVOKED — do not auto-announce (was spamming false "role removed" after grants).
  // If we just granted this role, put it back immediately.
  if (discord.peekRecentBotRoleChange(targetId, 'granted')) {
    try {
      await discord.addSubRole(targetId);
      kickSubscriberStore?.recordGrant?.(targetId, {
        kickUsername: link?.kickUsername || kickName,
        kickUserId: link?.kickUserId || null,
        active: true,
      ...
      console.log(
        `[discord-role-watch] Restored sub role for ${kickName} (${targetId}) after immediate strip`
      );
    } catch (error) {
      console.warn(
        `[discord-role-watch] restore failed for ${targetId}:`,
        error.message || error
      );
    }
    return false;
  ...
  // Keep local grant active unless a human remove is explicitly announced.
  if (announceRemoves) {
    kickSubscriberStore?.recordGrant?.(targetId, {
      kickUsername: link?.kickUsername || kickName,
      kickUserId: link?.kickUserId || null,
  ...
  console.log(
    `[discord-role-watch] Ignoring role remove for ${kickName} (${targetId}) — announce-removes off`
  );
  return false;
}
