...
async function handleEntry(entry, roleId, botUserId) {
  ...
  const grant = kickSubscriberStore?.getGrant?.(targetId);
  ...
  // REVOKED — if our records still say they should have the role, put it back.
  // (Do this even when the bot was the actor — chat revoke / race used to strip
  // roles while the dashboard still showed granted.)
  const shouldKeep =
    Boolean(grant?.active) ||
    discord.peekRecentBotRoleChange(targetId, 'granted');

  if (shouldKeep) {
    try {
      await discord.ensureSubRole(targetId);
      kickSubscriberStore?.recordGrant?.(targetId, {
        kickUsername: link?.kickUsername || grant?.kickUsername || kickName,
        kickUserId: link?.kickUserId || grant?.kickUserId || null,
        active: true,
        lastCheckedAt: new Date().toISOString(),
        revokeReason: null
      });
      console.log(
        `[discord-role-watch] Re-applied sub role for ${kickName} (${targetId}) — grant still active`
      );
    } catch (error) {
      console.warn(
  ...
  console.log(
    `[discord-role-watch] Ignoring role remove for ${kickName} (${targetId}) — no active grant`
  );
  return false;
}
