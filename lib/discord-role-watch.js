...
let roleLogicBlockQueued = false;
let lastRoleLogicBlockAt = 0;
...
function queueRoleLogicHardBlock(reason = 'rolelogic-remove', actorUserId = null) {
  if (String(process.env.DISCORD_BLOCK_ROLELOGIC || '1').trim() === '0') return;
  if (roleLogicBlockQueued) return;
  if (Date.now() - lastRoleLogicBlockAt < 30_000) return;
  roleLogicBlockQueued = true;
  setTimeout(() => {
    roleLogicBlockQueued = false;
    lastRoleLogicBlockAt = Date.now();

    // Soft demote first is useless while RoleLogic still syncs — remove it.
    const autoKick =
      String(process.env.DISCORD_KICK_ROLELOGIC || '1').trim() !== '0';
    const run = autoKick
      ? discord.removeRoleLogicPermanently({
          reason: `Auto-remove after ${reason}`,
          userId: actorUserId || discord.ROLELOGIC_BOT_USER_ID,
    ...
    Promise.resolve(run)
      .then((result) => {
        console.log(
          `[discord-role-watch] RoleLogic stop after ${reason}: ${result.message || result.error || result.reason}`
        );
      })
      .catch((error) => {
        console.warn(
          `[discord-role-watch] RoleLogic stop failed:`,
          error.message
        );
      });
  }, 400);
...
async function fetchAuditBundle(guildId) {
  ...
  const entries = Array.isArray(data?.audit_log_entries) ? data.audit_log_entries : [];
  const users = new Map(
...
function isRoleLogicActor(entry, usersById = null) {
  const actorId = String(entry?.user_id || '');
  if (!actorId) return false;
  if (
    actorId === String(discord.ROLELOGIC_BOT_USER_ID || '1389130157427785779')
  ) {
    return true;
  }
  const user = usersById?.get?.(actorId);
  const name = String(user?.username || user?.global_name || '');
  return /role\s*logic/i.test(name);
}

function actorLabel(entry, botUserId, usersById = null) {
  const actorId = String(entry?.user_id || '');
  if (!actorId) return 'unknown';
  if (botUserId && actorId === String(botUserId)) return 'na5ty-bot';
  if (isRoleLogicActor(entry, usersById)) return 'RoleLogic';
  return actorId;
}
...
async function handleEntry(entry, roleId, botUserId, usersById = null) {
  ...
  const actor = actorLabel(entry, botUserId, usersById);
  const actorUserId = String(entry?.user_id || '') || null;
  ...
  if (shouldKeep) {
    if (actor === 'RoleLogic') {
      queueRoleLogicHardBlock('rolelogic-strip-eligible', actorUserId);
    }
    try {
      // Small delay so RoleLogic's batch finishes before we put roles back,
      // then kick/ban RoleLogic so the next ~11m cycle can't happen.
      if (actor === 'RoleLogic') {
        await sleep(800);
      }
  ...
  if (actor === 'RoleLogic') {
    queueRoleLogicHardBlock('rolelogic-strip-no-grant', actorUserId);
    console.log(
      `[discord-role-watch] RoleLogic remove for ${kickName} (${targetId}) — removing RoleLogic from server`
    );
    return { handled: false, restored: false };
  }
...
async function pollOnce() {
  ...
  try {
    const { entries, users } = await fetchAuditBundle(cfg.guildId);
    ...
    for (const entry of sorted) {
      ...
      try {
        const result = await handleEntry(entry, cfg.subRoleId, botUserId, users);
        if (result?.handled) {
          eventsSeen += 1;
...
function getStatus() {
  return {
    ...
    roleLogicWarning:
      'RoleLogic is still removing Kick Supporter on a timer. Stop it: drag RoleLogic BELOW Kick Supporter in Server Settings → Roles, or disable the Kick rule at rolelogic.faizo.net, or kick RoleLogic. Until then it will keep fighting our restores.'
  };
}
