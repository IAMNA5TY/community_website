...
function trimStore(store) {
  ...
  if (Array.isArray(store.gifts) && store.gifts.length > MAX_GIFTS) {
    store.gifts = store.gifts.slice(-MAX_GIFTS);
  }
}

function getStoredViewerBalance(streamer, username) {
  const store = readStore();
  const streamerSlug = normalizeStreamer(streamer);
  const user = normalizeUsername(username);
  if (!streamerSlug || !user) return null;
  return (
    (store.viewerBalances || []).find(
      (row) =>
        streamerSlugMatches(streamerSlug, row.streamer) &&
        normalizeUsername(row.username) === user
    ) || null
  );
}

function setViewerBalance(streamer, username, points, { source = "sync" } = {}) {
  const streamerSlug = normalizeStreamer(streamer);
  const user = normalizeUsername(username);
  ...
  const store = readStore();
  if (!Array.isArray(store.viewerBalances)) store.viewerBalances = [];
  const idx = store.viewerBalances.findIndex(
    (row) =>
      streamerSlugMatches(streamerSlug, row.streamer) &&
      normalizeUsername(row.username) === user
  );
  ...
  if (idx >= 0) store.viewerBalances[idx] = entry;
  else store.viewerBalances.push(entry);
  if (store.viewerBalances.length > 5000) {
    store.viewerBalances = store.viewerBalances.slice(-5000);
  }
  writeStore(store);
  return entry;
}

function applyViewerBalanceDelta(streamer, username, delta, source = "command") {
  const current = getStoredViewerBalance(streamer, username);
  if (!current) return null;
  return setViewerBalance(streamer, username, current.points + Number(delta || 0), {
    source,
  });
}

function queueBalanceLookup(streamer, username) {
  const streamerSlug = normalizeStreamer(streamer);
  const user = normalizeUsername(username);
  if (!streamerSlug || !user) return;
  const store = readStore();
  if (!Array.isArray(store.pendingBalanceLookups)) store.pendingBalanceLookups = [];
  store.pendingBalanceLookups = store.pendingBalanceLookups.filter(
    (row) => Date.now() - new Date(row.at).getTime() < 60000
  );
  store.pendingBalanceLookups.push({
    streamer: streamerSlug,
    username: user,
    at: new Date().toISOString(),
  });
  if (store.pendingBalanceLookups.length > 200) {
    store.pendingBalanceLookups = store.pendingBalanceLookups.slice(-200);
  }
  writeStore(store);
}

function parseBalanceReply(content, expectedUsername = null) {
  const text = String(content || "").trim();
  if (!text) return null;
  const expected = normalizeUsername(expectedUsername);

  const patterns = [
    /@([a-z0-9_-]+)\b[^0-9]{0,48}?(\d{1,7})\s*(?:kick\s*)?points?\b/i,
    /\b([a-z0-9_-]+)\b[^0-9]{0,24}?(?:balance|points?)[:\s-]+(\d{1,7})\b/i,
    /\b([a-z0-9_-]+)\b[^0-9]{0,24}?(\d{1,7})\s*(?:kick\s*)?points?\b/i,
    /\byou(?:'re| are)?\s+(?:at|on)\s+(\d{1,7})\s*(?:kick\s*)?points?\b/i,
    /\byou have\s+(\d{1,7})\s*(?:kick\s*)?points?\b/i,
    /\bbalance[:\s]+(\d{1,7})\b/i,
  ];

  for (const pattern of patterns) {
    const match = text.match(pattern);
    if (!match) continue;
    if (match.length >= 3 && match[2] != null) {
      const user = normalizeUsername(match[1]);
      const points = Math.floor(Number(match[2]));
      if (!user || !Number.isFinite(points)) continue;
      if (expected && user !== expected) continue;
      return { username: user, points };
    }
    const points = Math.floor(Number(match[1]));
    if (!Number.isFinite(points) || !expected) continue;
    return { username: expected, points };
  }
  return null;
}

function isSiteBalancePromo(content) {
  const text = String(content || "");
  return (
    /only pixels rp/i.test(text) &&
    (/take the wheel/i.test(text) || /don.?t forget tip/i.test(text))
  );
}

function ingestBalanceReply(streamer, content) {
  if (isSiteBalancePromo(content)) return null;
  const streamerSlug = normalizeStreamer(streamer);
  const store = readStore();
  const pending = (store.pendingBalanceLookups || []).filter(
    (row) =>
      streamerSlugMatches(streamerSlug, row.streamer) &&
      Date.now() - new Date(row.at).getTime() <= 45000
  );

  // Prefer matching a pending balance requester.
  for (const row of pending.slice().reverse()) {
    const parsed = parseBalanceReply(content, row.username);
    if (!parsed) continue;
    setViewerBalance(streamerSlug, parsed.username, parsed.points, {
      source: "balance",
    });
    const nextStore = readStore();
    nextStore.pendingBalanceLookups = (nextStore.pendingBalanceLookups || []).filter(
      (item) =>
        !(
          streamerSlugMatches(streamerSlug, item.streamer) &&
          normalizeUsername(item.username) === parsed.username
        )
    );
  ...
  // Also accept explicit "@user … N points" even without a pending lookup.
  const explicit = parseBalanceReply(content, null);
  if (explicit?.username) {
    setViewerBalance(streamerSlug, explicit.username, explicit.points, {
      source: "balance",
    });
    return explicit;
  }
...
function recordChatMessage({
  ...
  const streamerSlug = normalizeStreamer(streamer);
  const chatter = resolveLinkedUsername(username);
  const text = String(content || "").trim();
  if (!streamerSlug || !chatter || !text) return null;

  const store = readStore();
  const id = messageId ? String(messageId).trim() : "";
  const atMs = new Date(createdAt).getTime() || Date.now();

  if (id) {
    const already = store.messages.some(
      (row) =>
        row.messageId &&
        String(row.messageId) === id &&
        streamerSlugMatches(streamerSlug, row.streamer)
    );
    if (already) return null;
  }

  // Collapse City-send echo vs Pusher. Two Kick ids = two real chats (points).
  const recent = store.messages.slice(-80);
  for (let i = recent.length - 1; i >= 0; i -= 1) {
    const row = recent[i];
    if (!streamerSlugMatches(streamerSlug, row.streamer)) continue;
    if (normalizeUsername(row.username) !== chatter) continue;
    if (String(row.content || "") !== text) continue;
    const dt = Math.abs((new Date(row.createdAt).getTime() || 0) - atMs);
    if (dt > 5000) continue;
    const rowId = row.messageId ? String(row.messageId) : "";
  ...
  if (control?.action === "balance") {
    queueBalanceLookup(streamerSlug, chatter);
  } else {
    const cost = control
      ? getCommandPointCost(control.action, { chatter })
      : 0;
    if (cost > 0) {
      applyViewerBalanceDelta(streamerSlug, chatter, -cost, "command");
    }
  }

  // Capture the in-game balance reply that follows a `balance` command.
  ingestBalanceReply(streamerSlug, text);
...
function licenseKeys(gameLicense) {
  const raw = String(gameLicense || "").trim();
  if (!raw) return [];
  const bare = raw.replace(/^license2?:/i, "");
  return [...new Set([raw, bare, `license:${bare}`, `license2:${bare}`])];
}

function getRegistrationByLicense(gameLicense) {
  const keys = licenseKeys(gameLicense);
  if (!keys.length) return null;
  const store = readStore();
  return (
    store.registrations.find((row) => keys.includes(String(row.gameLicense || "").trim())) || null
  );
}
...
function getRegistrationByTwitch(twitchUsername) {
  const login = normalizeUsername(twitchUsername);
  if (!login) return null;
  const store = readStore();
  return (
    store.registrations.find((row) => normalizeUsername(row.twitchUsername) === login) || null
  );
}

function resolveLinkedUsername(username) {
  const normalized = normalizeUsername(username);
  return getRegistrationByTwitch(normalized)?.kickUsername || normalized;
}

function linkTwitchUsername(kickUsername, twitchUsername) {
  const kick = normalizeUsername(kickUsername);
  const twitch = normalizeUsername(twitchUsername);
  if (!kick) throw new Error("Kick username is required");
  if (!twitch) throw new Error("Twitch username is required");

  const store = readStore();
  let entry = store.registrations.find((row) => row.kickUsername === kick);
  if (!entry) {
    entry = {
      kickUsername: kick,
      displayName: kick,
  ...
  const taken = store.registrations.find(
    (row) => normalizeUsername(row.twitchUsername) === twitch && row.kickUsername !== kick
  );
  if (taken) {
    throw new Error(`Twitch @${twitch} is already linked to another Kick account`);
  }

  entry.twitchUsername = twitch;
  entry.twitchLinkedAt = new Date().toISOString();
  entry.updatedAt = new Date().toISOString();
  writeStore(store);
  return entry;
}

function attachGameLicenseIfEmpty(kickUsername, gameLicense) {
  const kick = normalizeUsername(kickUsername);
  const keys = licenseKeys(gameLicense);
  if (!kick || !keys.length) return getRegistration(kick);
  const store = readStore();
  const entry = store.registrations.find((row) => row.kickUsername === kick);
  if (!entry) return null;
  if (entry.gameLicense) return entry;
  const taken = store.registrations.find((row) => keys.includes(String(row.gameLicense || "").trim()));
  if (taken && taken.kickUsername !== kick) return entry;
  entry.gameLicense = keys[0];
  entry.updatedAt = new Date().toISOString();
  writeStore(store);
  return entry;
}

function listTwitchLinks() {
  return readStore()
    .registrations.filter((row) => normalizeUsername(row.twitchUsername))
    .map((row) => ({
      kickUsername: row.kickUsername,
      twitchUsername: normalizeUsername(row.twitchUsername),
...
function getControlEvents(streamer, { afterId = 0, limit = 20, init = false } = {}) {
  ...
  const events = eventsForStreamer
    ...
    .slice(0, lim)
    .map((row) => {
      const pointCost = getCommandPointCost(row.action, {
        chatter: row.chatter_username,
      });
      return {
        id: row.id,
        streamer: streamerSlug,
...
/**
 * 24h chat-derived points only (not the synced Only Pixels RP balance).
 * Regular chat earns 1; paid movement commands spend pointCost.
 */
function computeViewerChatPoints(streamer, kickUsername, { hours = 24 } = {}) {
  const streamerSlug = normalizeStreamer(streamer);
  const username = normalizeUsername(kickUsername);
  if (!streamerSlug || !username) {
    return {
      streamer: streamerSlug,
      kickUsername: username,
      points: 0,
  ...
  for (const row of store.messages) {
    ...
    if (normalizeUsername(row.username) !== username) continue;

    messageCount += 1;
    const control = parseChatControlAction(row.content);
    const cost = control
      ? getCommandPointCost(control.action, { chatter: username })
      : 0;
    if (cost > 0) {
      spent += cost;
    } else {
  ...
  return {
    ...
    source: "computed",
    hours,
    gift_kicks_all_time: sumGiftKicks(streamerSlug, username, 0).total_kicks,
  };
}

/**
 * Kick Points on a channel:
 * Prefer live chat math. Stored balances from old gift sync inflated people
 * (e.g. 15k–20k with 1 message). Only trust stored when it looks like a real
 * chat wallet (near computed) or an explicit spend that is not above chat.
 */
function getViewerPointsOnChannel(streamer, kickUsername, { hours = 24 } = {}) {
  const chat = computeViewerChatPoints(streamer, kickUsername, { hours });
  const stored = getStoredViewerBalance(chat.streamer, chat.kickUsername);
  const storedPoints = stored ? Math.floor(Number(stored.points)) : NaN;
  if (Number.isFinite(storedPoints) && storedPoints > 0) {
    const source = String(stored.source || "");
    const closeToChat =
      Math.abs(storedPoints - chat.points) <= Math.max(25, chat.points * 0.25);
    const commandLike =
      source === "command" || source === "balance" || source === "game";
    if (commandLike && (closeToChat || storedPoints <= chat.points)) {
      return {
        streamer: chat.streamer,
        kickUsername: chat.kickUsername,
        points: storedPoints,
        earned: chat.earned,
        spent: chat.spent,
        message_count_24h: chat.message_count_24h,
        source: source || "balance",
        syncedAt: stored.updatedAt || null,
        hours: chat.hours,
        gift_kicks_all_time: chat.gift_kicks_all_time,
      };
    }
  }
...
function getRecentMessages(streamer, { hours = 24, limit = 50 } = {}) {
  ...
  for (const row of rows) {
    ...
    if (prev) {
      const sameUser =
        normalizeUsername(prev.username) === normalizeUsername(row.username);
      const sameText = String(prev.content || "") === String(row.content || "");
      const dt = Math.abs(
        (new Date(row.createdAt).getTime() || 0) -
          (new Date(prev.createdAt).getTime() || 0)
      );
      if (sameUser && sameText && dt <= 5000) continue;
    }
    deduped.push(row);
  }
...
function getRewardsSummary(kickUsername) {
  ...
  let totalPoints24h = 0;
  let totalSpent24h = 0;
  let activeChannels = 0;

  for (const streamer of getRewardsScanStreamers(24)) {
    const pointsRow = getViewerPointsOnChannel(streamer, username, { hours: 24 });
    if (pointsRow.message_count_24h <= 0) continue;
    ...
    activeChannels += 1;
    totalMessages24h += pointsRow.message_count_24h;
    totalPoints24h += pointsRow.points;
    totalSpent24h += pointsRow.spent;
    byStreamer[streamer] = {
      message_count_24h: pointsRow.message_count_24h,
      ...
      chat_commands: getChatCommandsForStreamer(streamer, { chatter: username }),
    };
  }

  return {
    kickUsername: username,
    twitchUsername: getRegistration(username)?.twitchUsername || null,
    registered: Boolean(getRegistration(username)),
    total_messages_24h: totalMessages24h,
    total_points_24h: totalPoints24h,
    total_spent_24h: totalSpent24h,
    active_channels: activeChannels,
    streamers: byStreamer,
  };
}

/** Streamer.bot Kick channel-point redeem → FiveM (cash OR carhub coins; NOT shop / NOT chat tip). */
function enqueueChannelCash(payload) {
  const kickUsername = normalizeUsername(payload?.kickUsername || payload?.username);
  const amount = Math.max(0, Math.floor(Number(payload?.amount) || 0));
  const rewardTitle = String(payload?.rewardTitle || payload?.title || "").trim();
  const streamer = normalizeStreamer(payload?.streamer || "na5ty");
  const redemptionId = String(payload?.redemptionId || payload?.id || "").trim();
  let rewardKind = String(payload?.rewardKind || payload?.kind || "cash")
    .trim()
    .toLowerCase();
  if (
    rewardKind !== "carhub" &&
  ...
  const itemName = String(payload?.itemName || payload?.item || "")
    .trim()
    .toUpperCase();
  if (!kickUsername || amount < 1) {
    throw new Error("kickUsername and amount (>=1) required");
  }
  if (
    (rewardKind === "item" || rewardKind === "weapon" || rewardKind === "arp") &&
    !itemName &&
    rewardKind !== "arp"
  ) {
    throw new Error("itemName required for item/weapon redeems");
  }

  const store = readStore();
  if (!Array.isArray(store.channelCashQueue)) store.channelCashQueue = [];
  if (!store.nextChannelCashId) store.nextChannelCashId = 1;

  // Dedupe by Kick redemption id when present
  if (redemptionId) {
    const existing = store.channelCashQueue.find(
      (row) => row.redemptionId && row.redemptionId === redemptionId
    );
    if (existing) return existing;
  }

  const row = {
    id: store.nextChannelCashId++,
    kickUsername,
    amount,
    rewardKind,
    itemName: itemName || (rewardKind === "arp" ? "WEAPON_ARP_PHOENIX" : null),
    rewardTitle,
    streamer,
  ...
  store.channelCashQueue.push(row);
  // Keep last 500
  if (store.channelCashQueue.length > 500) {
    store.channelCashQueue = store.channelCashQueue.slice(-500);
  }
  writeStore(store);
  return row;
}

function listPendingChannelCash(limit) {
  const store = readStore();
  const cap = Math.min(50, Math.max(1, Number(limit) || 20));
  const rows = (store.channelCashQueue || []).filter((r) => r.status === "pending");
  return rows.slice(0, cap);
}

function completeChannelCash(id, result) {
  const store = readStore();
  const row = (store.channelCashQueue || []).find((r) => Number(r.id) === Number(id));
  if (!row) return null;
  const ok = result && result.ok !== false && result.success !== false;
  row.status = ok ? "paid" : "failed";
  row.paidAt = new Date().toISOString();
  row.error = ok ? null : String(result?.error || result?.reason || "failed");
  row.playerId = result?.playerId != null ? result.playerId : null;
  writeStore(store);
  return row;
}

/** Hard wipe — stuck / test / mis-typed redeems. */
function clearChannelCashQueue() {
  const store = readStore();
  const before = Array.isArray(store.channelCashQueue) ? store.channelCashQueue.length : 0;
  store.channelCashQueue = [];
  writeStore(store);
...
module.exports = {
  ...
  linkGameLicense,
  linkTwitchUsername,
  attachGameLicenseIfEmpty,
  getRegistrationByLicense,
  licenseKeys,
  getRegistration,
  getRegistrationByTwitch,
  resolveLinkedUsername,
  listTwitchLinks,
  listRegistrations,
  recordChatMessage,
  recordKicksGifted,
  ...
  sumGiftKicks,
  getViewerEarnTotals,
  getViewerPointsOnChannel,
  computeViewerChatPoints,
  getStoredViewerBalance,
  ingestBalanceReply,
  isSiteBalancePromo,
  setViewerBalance,
  applyViewerBalanceDelta,
