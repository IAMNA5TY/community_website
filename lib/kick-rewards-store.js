...
function upsertMonitoredStreamer(slug, broadcasterId = null, options = {}) {
  ...
  if (!partnerRegistry.isApproved(normalized)) {
    throw new Error(`@${normalized} is not approved for Kick City partnership`);
  }
...
function recordChatMessage({ streamer, username, content, createdAt = new Date().toISOString() }) {
  ...
  if (!streamerSlug || !chatter || !text || partnerRegistry.isBanned(chatter)) return null;
...
function recordKicksGifted({ streamer, donor, amount, createdAt = new Date().toISOString() }) {
  ...
  if (!streamerSlug || !donorName || kicks < 1 || partnerRegistry.isBanned(donorName)) return;
...
function registerKickUsername(kickUsername, meta = {}) {
  ...
  if (partnerRegistry.isBanned(username)) {
    throw new Error("This Kick account is banned from Kick City");
  }
...
function getRegistrationByLicense(gameLicense) {
  ...
  return entry && !partnerRegistry.isBanned(entry.kickUsername) ? entry : null;
}

function getRegistration(kickUsername) {
  const username = normalizeUsername(kickUsername);
  if (partnerRegistry.isBanned(username)) return null;
  const store = readStore();
...
function getChatters(streamer, { hours = 24, limit = 200 } = {}) {
  ...
  for (const row of store.messages) {
    ...
    if (partnerRegistry.isBanned(user)) continue;
...
function getControlEvents(streamer, { afterId = 0, limit = 20, init = false } = {}) {
  ...
  const eventsForStreamer = store.controlEvents.filter((row) =>
    ...
    !partnerRegistry.isBanned(row.chatter_username)
  );
...
function sumGiftKicks(streamer, donor, hours = 0) {
  ...
  const total = store.gifts.reduce((sum, row) => {
    ...
    if (partnerRegistry.isBanned(row.donor)) return sum;
...
/** Keep Pusher on env/bootstrap/live/recent — demote the rest. */
...
/** Pusher targets: env + bootstrap + chatPriority/live only (not all Kick City rows). */
function getChatMonitorSlugs() {
  const removed = getRemovedPartnerSet();
  const slugs = new Set();
  for (const application of partnerRegistry.listApplications()) {
    const key = normalizeStreamer(application.kickUsername);
    if (application.status === "approved" && key && !removed.has(key)) slugs.add(key);
  }
  const store = readStore();
  for (const row of store.monitoredChannels || []) {
    const slug = normalizeStreamer(row.slug);
    if (!slug || removed.has(slug) || !partnerRegistry.isApproved(slug)) continue;
