 ...
 * Prefer the live `balance` command value (synced from chat / game).
 * Fallback: regular chat earns 1, paid movement commands spend pointCost.
 */
function getViewerPointsOnChannel(streamer, kickUsername, { hours = 24 } = {}) {
  ...
  const stored = getStoredViewerBalance(streamerSlug, username);
  if (stored && Number.isFinite(Number(stored.points))) {
    return {
      streamer: streamerSlug,
      kickUsername: username,
      points: Math.max(0, Math.floor(Number(stored.points))),
      earned,
      spent,
