...
function upsertMonitoredStreamer(slug, broadcasterId = null, options = {}) {
  ...
  if (existing) {
    ...
    store.monitoredChannels.push({
      slug: normalized,
      broadcasterId: id,
      chatPriority: options.chatPriority === true,
  ...
  const row = store.monitoredChannels.find(
    (item) => normalizeStreamer(item.slug) === normalized
  );
  return {
    slug: normalized,
...
function listMonitoredChannels() {
  ...
  // Env channels (e.g. na5ty) even if somehow missing from store.
  for (const slug of parsePartnerSlugList()) {
    if (!slug || seen.has(slug)) continue;
    seen.add(slug);
    rows.push({
...
// Env channels (na5ty) + optional KICK_REWARD_PARTNER_SLUGS + store (Only Pixels / FiveM sync).
// Community bootstrap restores known partners into the store after deploys (removes still stick).
const COMMUNITY_PARTNER_BOOTSTRAP = [
  "andy1993",
  "vikinggaming94",
  "d0sil",
  "devilmaykill579",
...
function ensureDefaultPartners() {
  ...
  for (const slug of parsePartnerSlugList()) {
    ...
    added.push(upsertMonitoredStreamer(slug, null, { chatPriority: true }));
  }

  for (const slug of COMMUNITY_PARTNER_BOOTSTRAP) {
    const key = normalizeStreamer(slug);
    if (!key || removed.has(key)) continue;
    added.push(upsertMonitoredStreamer(key, null, { chatPriority: true }));
  }

  // Promote recent Only Pixels / admin adds (not a bulk FiveM dump of 40+ rows).
  const store = readStore();
  const twoHoursAgo = Date.now() - 2 * 60 * 60 * 1000;
  const recent = (store.monitoredChannels || []).filter((row) => {
    const at = row.addedAt ? new Date(row.addedAt).getTime() : 0;
    return at >= twoHoursAgo && row.broadcasterId;
  });
  if (recent.length > 0 && recent.length <= 15) {
    for (const row of recent) {
      const key = normalizeStreamer(row.slug);
      if (!key || removed.has(key)) continue;
      added.push(
        upsertMonitoredStreamer(key, row.broadcasterId, { chatPriority: true })
      );
    }
  }
...
/** Slugs that should get Pusher chat capture (not every Kick City DB row). */
function getChatMonitorSlugs() {
  const slugs = new Set(parsePartnerSlugList());
  for (const slug of COMMUNITY_PARTNER_BOOTSTRAP) {
  ...
  for (const row of store.monitoredChannels || []) {
    const slug = normalizeStreamer(row.slug);
    if (!slug) continue;
    if (row.chatPriority === true) slugs.add(slug);
  }
