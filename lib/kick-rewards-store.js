...
function ensureDefaultPartners() {
  ...
  // Promote every stored partner to chat — no manual Enable click required.
  const store = readStore();
  for (const row of store.monitoredChannels || []) {
    const key = normalizeStreamer(row.slug);
    if (!key || removed.has(key)) continue;
...
/** Every monitored partner gets Pusher chat (connects are staggered to avoid Kick rate limits). */
function getChatMonitorSlugs() {
  const removed = getRemovedPartnerSet();
  const slugs = new Set();
  for (const slug of getMonitoredStreamers()) {
    const key = normalizeStreamer(slug);
