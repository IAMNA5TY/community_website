...
/** Pusher targets: chatPriority / liveChat only (not every approved partner). */
function getChatMonitorSlugs() {
  ...
  // Env extras always monitored (bootstrap / emergency).
  for (const raw of String(process.env.KICK_MONITOR_EXTRA_SLUGS || "").split(",")) {
    const slug = normalizeStreamer(raw);
