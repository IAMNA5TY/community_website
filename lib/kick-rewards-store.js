...
function ensureDefaultPartners() {
  ...
  // Collapse stevens_gaming / stevens-gaming style duplicates.
  dedupePartnerSlugVariants();
  // Don't force every Kick City row onto Pusher — only priority / live / recent.
...
function pruneChatPriorityToActive() {
  const keep = new Set(parsePartnerSlugList());
  for (const slug of COMMUNITY_PARTNER_BOOTSTRAP) {
...
function getChatMonitorSlugs() {
  ...
  for (const slug of COMMUNITY_PARTNER_BOOTSTRAP) {
    const key = normalizeStreamer(slug);
    if (key && !removed.has(key)) slugs.add(key);
  }
  const store = readStore();
  for (const row of store.monitoredChannels || []) {
    const slug = normalizeStreamer(row.slug);
