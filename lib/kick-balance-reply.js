...
function formatBalanceReply(username, points, streamerSlug) {
  ...
  if (n <= 0) {
    return `@${username} has 0 points in Only Pixels RP on ${who}'s stream. Chat to stack some, then take the wheel — left, right, or forward.`;
  }
  return `@${username} has ${n} point${n === 1 ? "" : "s"} in Only Pixels RP — ready to take the wheel on ${who}. left, right, or forward.`;
}
